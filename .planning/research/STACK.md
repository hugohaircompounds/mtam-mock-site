...
**Domain:** Single-page self-contained HTML demo — hair extension education platform homepage
**Researched:** 2026-03-30
**Confidence:** HIGH (all CDN URLs verified, version numbers confirmed from official sources)
...
**Recommended pairing: DM Serif Display + Manrope**

This pairing serves the MTAM brand specifically:
...
Google Fonts CDN link (both fonts, single request):
...
<!-- Icons: Phosphor (regular + fill weights only — sufficient for this demo) -->
...
**If the presentation venue has unreliable internet:**
...
**If the demo needs FAQ/accordion interactivity:**
...
**If MTAM brand color needs to stay strictly black/white:**
...
**If the demo is being screenshotted or recorded rather than clicked through:**
...
For visual premium quality in a single HTML file, these CSS custom properties should be set at the `:root` level and referenced throughout:

```css
:root {
  --color-bg: #0f0f0f;           /* near-black background */
  --color-surface: #1a1a1a;      /* card/section backgrounds */
  --color-accent: #C9A84C;       /* warm gold — elegance signal */
  --color-text-primary: #f5f5f5; /* near-white body text */
  --color-text-muted: #9ca3af;   /* gray-400 for captions, metadata */
...
This dark-background aesthetic matches MTAM's existing black/white brand, creates presentation contrast (easy to read on projector), and supports glassmorphism card effects without clashing.
...
*Stack research for: Self-contained single HTML file homepage demo — JTBD-driven hair extension education platform redesign*
*Researched: 2026-03-30*
